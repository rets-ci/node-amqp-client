  ...
  function startWorker( queue, options, work) {
    ...
    amqpConn.createChannel(function(err, ch) {
      ...
      if( options.exchange ) {
        ...
        if( options.routingKeyMatches ) {
          if( typeof options.routingKeyMatches !== 'object' ) {
            options.routingKeyMatches = [ options.routingKeyMatches ];
          }
          options.routingKeyMatches = _.map( options.routingKeyMatches, function(o){return o} );
          debug( "[AMQP] routing key matches",require('util').inspect(options.routingKeyMatches, {showHidden: false, depth: 10, colors: true}));
          options.routingKeyMatches.forEach( function( v ) {
